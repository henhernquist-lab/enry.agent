...
export function GradeCalculator({ onClose }: { onClose: () => void }) {
  ...
  // For each class: calculate needed final score to achieve target GPA for that class
  const results = classes.map((c) => {
    const fw = c.finalWeight / 100
    // target percent to earn the gpa contribution we want ≈ target in letter form
    // Simplification: solve for final score given target grade in this class
    // target_grade = current * (1 - fw) + final * fw  →  final = (target_grade - current * (1-fw)) / fw

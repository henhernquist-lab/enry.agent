  ...
  return (
    ...
    >
      ...
      <div className="grid grid-cols-2 gap-x-4 gap-y-2.5 border-t border-border/60 pt-3">
        <Metric icon={Clock} label="Avg latency" value={formatLatency(health.avgLatencyMs)} />
        <Metric icon={CheckCircle2} label="Success rate" value={`${health.successRate}%`} valueClass={health.successRate >= 95 ? 'text-primary' : 'text-foreground'} />
        <Metric icon={AlertCircle} label="Error rate" value={`${health.errorRate.toFixed(1)}%`} valueClass={health.errorRate > 5 ? 'text-warning' : 'text-foreground'} />
        ...
        <Metric icon={CalendarClock} label="Last success" value={relativeTime(health.lastSuccessAt)} valueClass={health.status === 'offline' ? 'text-muted-foreground/50' : 'text-foreground'} />
      </div>

...
// Fires workflow_dispatch for enry-cruise-goal.yml — the scanfix actuator.
// Unlike dispatchScan, the workflow filename isn't hardcoded — goal runs live
// in a separate file from the scan workflow (different permissions: goal runs
// stay contents:read too, since writes go through
// /api/cruise/goal-runs/[id]/apply, but keeping them as separate files keeps
// the two dispatch surfaces independently versionable).

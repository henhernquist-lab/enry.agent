...
// The compare-and-swap primitive itself now lives in session-cas.ts (shared
// with Learn's learn-ops.ts, same row pattern, same discipline) — this file
// just re-exports it under its established name for existing call sites, and
// keeps the terminal-session-typed convenience wrapper.
export { casUpdateSessionPayload } from '../session-cas'
import { casUpdateSessionPayload as casUpdate, loadSessionPayload as loadPayload } from '../session-cas'

async function loadSessionPayload(sessionId: string, userId: string): Promise<TerminalSessionPayload | null> {
  return loadPayload<TerminalSessionPayload>(sessionId, userId)
}

async function saveSessionPayload(sessionId: string, userId: string, patch: Partial<TerminalSessionPayload>): Promise<void> {
  await casUpdate<TerminalSessionPayload>(sessionId, userId, () => patch)
}
